...
def drawPlots(plots, mode, dataMCScale):
  for log in [False, True]:
    plot_directory_ = os.path.join(plot_directory, 'test', args.plot_directory, 'RunIII', mode + ("_log" if log else ""), args.selection)
    for plot in plots:
...
def make_jets( event, sample ):
    ...
    event.maxBTagDisc = max([j['btagDeepB'] for j in event.jets])
...
for i_mode, mode in enumerate(allModes):
    ...
    plots.append(Plot( name = 'Btagging_discriminator_value' , texX = 'DeepB l1' , texY = 'Number of Events',
        attribute = lambda event, sample: event.JetGood_btagDeepB[0],
        binning = [20,0,1],
    ))

    plots.append(Plot( name = 'Btagging_discriminator_value' , texX = 'DeepB l2' , texY = 'Number of Events',
        attribute = lambda event, sample: event.JetGood_btagDeepB[1],
        binning = [20,0,1],
    ))

    plots.append(Plot( name = 'Btagging_discriminator_value' , texX = 'DeepB max' , texY = 'Number of Events',
        attribute = lambda event, sample: event.JetGood_maxBTag[0],
        binning = [20,0,1],
    ))
