...
samples = [TTbb_SMonly, TTbb_cQQ1_lin, TTbb_cQQ1_quad]
stack = Stack(samples)
for sample in samples : sample.style = styles.fillStyle(sample.color)
#for sample in samples : sample.weight = lambda event, sample: sample.xsec/10000
...
else: weight = None #lambda event, sample: event.genWeight if sample.isData else event.genWeight

# Read variables and sequences
read_variables = [
    "GenMet_pt/F", "GenMet_phi/F", 
    "nGenJet/I", "GenJet[pt/F,eta/F,phi/F,matchBParton/I]", 
    "LHEPart_phi/F","LHEPart_pdgId/I",
...
# Use some defaults
Plot.setDefaults(stack = stack, weight = staticmethod(weight), addOverFlowBin=None)
...
plots.append(Plot( name = 'ht_course',
  texX = 'H_{T}', texY = 'Number of Events',
  attribute = lambda event, sample: sum( [j['pt'] for j in event.jets]),
  binning=[10,500,2500],
))

#plots.append(Plot( name = 'initial_state_pdgId',
#  texX = 'pdgId', texY = 'Number of Events',
#  attribute = lambda event, sample: event.LHEPart_pdgId,# if event.LHEPart_phi==0 else 0,
#  binning=[28,-6,22],
#))
...
def drawPlots(plots, subDirectory=''):
  for log in [False, True]:
    plot_directory_ = os.path.join(plot_directory, "analysisPlots" ,args.plot_directory)
    ...
    for plot in plots:
      ...
      plotting.draw(plot,
	    ...
	    ratio = None,#{'histos':[(0,1),(0,2)], 'yRange':(0.1,1.9)},# if args.WC is not None else {'histos':[(0,0) ], 'yRange':(0.1,1.9)},
