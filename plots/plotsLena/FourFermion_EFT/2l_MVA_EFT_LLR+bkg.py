...
assert len(args.labels)==len(args.models) or args.labels == None, "need same number of labels and models"
bkg = "TT_2L"
...
# logging.info("calculating new weights, reweighting to integrated luminosity")
# W_0     = args.lumi * config.xsec[sample] * 1000 * y[:,0]/ config.total_genWeight[sample]
# W_0     = lumi * config.xsec[sample] * 1000 * y[:,0]/ np.sum(y[:,0])

# W_0_bkg = args.lumi * config.xsec[bkg] * 1000 * y_bkg[:,0]/ config.total_genWeight[bkg]
# W_0_bkg = lumi * config.xsec[bkg] * 1000 * y_bkg[:,0]/ np.sum(y_bkg[:,0])

# store new weights W_0 as a 4th position in y and y_bkg -> w and w_bkg
# w = np.zeros((len(y[:,0]),len(y[0,:])+1))
# w[:,:-1] = y 
# w[:,-1]  = W_0
...
assert not np.isnan( np.sum( w )+np.sum(w_bkg) ), logging.info("found NaNs in DNN truth values!")
...
if not (args.combine_models):  
    ...
    for idx, model in enumerate(args.models):
        ...
        k = 0
        for theta in theta_: 
            w_sm   =   np.concatenate((y[:,0],w_bkg[:,0]))
            stack_weights = np.concatenate((y[:,0],w_bkg[:,0])) + theta * np.concatenate((y[:,index_lin[model]],w_bkg[:,1]))+ theta**2 * np.concatenate((y[:,index_quad[model]],w_bkg[:,2]))
            ...
            if args.shape_effects_only: w_bsm  = stack_weights  #/np.sum(stack_weights) #* stack_weights
            else: 
                w_bsm  = stack_weights# /np.sum(np.concatenate((y[:,0],w_bkg[:,0]))) #*  stack_weights
                # if (Z[model+"_label"]=='ParticleNet'):
    ...
    for h in range(len(theta_)): 
        for g in range(len(theta_)): 
            ...
            w_sm   =   lumi /np.sum(y[:,-1]+w_bkg[:,-1]) * args.sample_weight * (y[:,-1]+w_bkg[:,-1])
            t_theta = 1
         
            stack_weights = y[:,-1]+w_bkg[:,-1] #SM weight
            for idx, model in enumerate(args.models):
                stack_weights = stack_weights + theta__[idx] * (y[:,index_lin[model]]+w_bkg[:,1]) + theta__[idx]**2 * (y[:,index_quad[model]]+w_bkg[:,2])
            ...
            stack_weights = stack_weights + theta_0*theta_1 * (y[:,index_mixed]+w_bkg[:,1])
            if args.shape_effects_only: w_bsm  =   lumi /np.sum(stack_weights) * args.sample_weight * stack_weights 
            else: w_bsm  =   lumi /np.sum(y[:,-1]+w_bkg[:,-1]) * args.sample_weight * stack_weights
