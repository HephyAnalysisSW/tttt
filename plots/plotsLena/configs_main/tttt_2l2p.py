...
def make_leptons(event, sample):
    ...
    dR_vals = sorted([deltaR(event.bJets[i], event.leptons[j]) for i in range(len(list(event.bJets))) for j in range(len(list(event.leptons)))])
    if len(dR_vals)>=2:
...
# global event properties
    ...
    # the working points are already implemented in isBJet
     "mva_nBTagJetL"             :(lambda event, sample: len(list(filter( lambda j:isBJet(j, WP="loose"),  event.jets )))),
     "mva_nBTagJetM"             :(lambda event, sample: len(list(filter( lambda j:isBJet(j, WP="medium"), event.jets )))),
     "mva_nBTagJetT"             :(lambda event, sample: len(list(filter( lambda j:isBJet(j, WP="tight"),  event.jets )))),
