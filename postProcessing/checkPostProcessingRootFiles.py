...
def get_parser():
    ...
    argParser.add_argument('--file',       action='store', type=str, default='nanoPostProcessing_UL20_Summer16', help="postprocessing sh file to check")
...
def getDataDictList( filepath ):
    ...
    for line in ppLines:
        ...
        era     = filterEmpty( line.split("--era ")[1].split(" ") )[0]
        ...
        dictList.append( { "skim":skim, "era":era, "dir":dir, "sample":sample, "nFiles":int(nFiles), "command":command} )

    return dictList

# Load File
logger.info( "Now running on pp file %s and checking in directory %s", args.file, args.data_directory)
file          = os.path.expandvars( "$CMSSW_BASE/src/tttt/postProcessing/%s.sh" % args.file )
...
for ppEntry in dictList:
    ...
    dirPath = os.path.join( args.data_directory, ppEntry["dir"], ppEntry["era"], ppEntry["skim"], sample+postfix  )
