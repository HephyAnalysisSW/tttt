...
if hasattr( sample, "reweight_pkl" ):
    ...
    basepoint_coordinates = [[d[v] for v in weightInfo.variables] for d in [interpret_weight(w[0]) for w in weightInfo_data]]
...
read_variables = list(map(TreeVariable.fromString, [ 'MET_pt/F', 'MET_phi/F', 'run/I', 'luminosityBlock/I', 'event/l', 'PV_npvs/I', 'PV_npvsGood/I'] ))
if options.era == "2017":
    read_variables += list(map(TreeVariable.fromString, [ 'METFixEE2017_pt/F', 'METFixEE2017_phi/F', 'METFixEE2017_pt_nom/F', 'METFixEE2017_phi_nom/F']))
    if isMC:
        read_variables += list(map(TreeVariable.fromString, [ 'METFixEE2017_pt_jesTotalUp/F', 'METFixEE2017_pt_jesTotalDown/F', 'METFixEE2017_pt_jerUp/F', 'METFixEE2017_pt_jerDown/F', 'METFixEE2017_pt_unclustEnDown/F', 'METFixEE2017_pt_unclustEnUp/F', 'METFixEE2017_phi_jesTotalUp/F', 'METFixEE2017_phi_jesTotalDown/F', 'METFixEE2017_phi_jerUp/F', 'METFixEE2017_phi_jerDown/F', 'METFixEE2017_phi_unclustEnDown/F', 'METFixEE2017_phi_unclustEnUp/F', 'METFixEE2017_pt_jer/F', 'METFixEE2017_phi_jer/F']))
# else:
...
if isMC:
    read_variables += list(map(TreeVariable.fromString, [ 'GenMET_pt/F', 'GenMET_phi/F' ]))
...
def filler( event ):
    ...
    clean_jets_acc = [j for j in clean_jets if abs(j['eta'])<2.4]
    jets         = [j for j in clean_jets_acc if j['pt']>30]
    bJets        = [j for j in jets if isBJet(j, tagger=b_tagger, year=options.era) and abs(j['eta'])<=2.4]
    nonBJets     = [j for j in jets if not ( isBJet(j, tagger=b_tagger, year=options.era) and abs(j['eta'])<=2.4 )]
    ...
    if addSystematicVariations:
        for var in ['jesTotalUp', 'jesTotalDown', 'jerUp', 'jerDown', 'unclustEnUp', 'unclustEnDown']: # don't use 'jer' as of now
            ...
            if not var.startswith('unclust'):
                ...
                jets_sys[var]       = [j for j in clean_jets_acc if j['pt_'+var]>30]
                bjets_sys[var]      = [j for j in jets_sys[var] if isBJet(j) and abs(j['eta'])<2.4]
                nonBjets_sys[var]   = [j for j in jets_sys[var] if not ( isBJet(j) and abs(j['eta'])<2.4)]
    ...
    if isSingleLep or isTriLep or isDiLep:
        event.nGoodMuons      = len([l for l in leptons if abs(l['pdgId'])==13])
        event.nGoodElectrons  = len([l for l in leptons if abs(l['pdgId'])==11])
    ...
    if isMC:
        ...
        for var in btagEff.btagWeightNames:
            if var!='MC':
                setattr(event, 'reweightBTag_'+var, btagEff.getBTagSF_1a( var, bJets, [j for j in nonBJets if abs(j['eta'])<2.4] ) )
...
for ievtRange, eventRange in enumerate( eventRanges ):
    ...
    for formula in list(treeFormulas.keys()):
    ...
    while reader.run():
        ...
        if sample.isData:
            if maker.event.jsonPassed_:
                if reader.event.run not in list(outputLumiList.keys()):
