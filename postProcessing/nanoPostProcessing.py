...
from math import sqrt, atan2, sin, cos, sinh
...
# tttt
from tttt.Tools.helpers             import closestOSDLMassToMZ, deltaR, deltaR2, deltaPhi, bestDRMatchInCollection, nonEmptyFile, getSortedZCandidates, cosThetaStar, m3, getMinDLMass
...
branchKeepStrings_MC = [ "Generator_*", "GenPart_*", "nGenPart", "genWeight", "Pileup_nTrueInt","GenMET_*", "nGenJet", "GenJet_*", "genTtbarId", "PSWeight", "LHE_HT*"]
...
minJetPt     = 25
maxJetAbsEta = 2.4
jetVars         = ['pt/F', 'chEmEF/F', 'chHEF/F', 'neEmEF/F', 'muEF/F', 'neHEF/F', 'rawFactor/F', 'eta/F', 'phi/F', 'jetId/I', 'btagDeepB/F', 'btagDeepCvB/F', 'btagDeepCvL/F', 'btagDeepFlavB/F', 'btagDeepFlavCvB/F', 'btagDeepFlavCvL/F', 'btagDeepFlavQG/F', 'btagCSVV2/F', 'area/F', 'pt_nom/F', 'corr_JER/F', 'qgl/F'] + jetCorrInfo
jesUncertainties = [
    "Total",
    "AbsoluteMPFBias",
    "AbsoluteScale",
...
if not options.skipNanoTools:
    ...
    for f in sample.files:
        JMECorrector = createJMECorrector(
            ...
            runPeriod   = runPeriod,
            jesUncert   = ",".join(jesUncertainties),# if addSystematicVariations else None,
            jetType     = "AK4PFchs",
...
def filler( event ):
    ...
    if sample.isMC:
        ...
        # sgn_top = 0
        # for gen in gPart:
        #     if ((gen['statusFlags']&(1<<12))!=0 and abs(gen['pdgId'])==6):
    ...
    for iLep, lep in enumerate(leptons):
        ...
        for j in analysis_jets:
            if not j['isNominal']: continue # compute topLeptonMVA from jets that satisfy the nominal pt cut
            deta = j['eta'] - lep['eta']
    ...
    if addSystematicVariations:
        for var_ in jesUncertainties: # don't use 'jer' as of now
            for upOrDown in ["Up", "Down"]:
                ...
                var                 = "jes"+var_+upOrDown
                jets_sys[var]       = filter(lambda j:j['pt_'+var]>minJetPt, jets)
