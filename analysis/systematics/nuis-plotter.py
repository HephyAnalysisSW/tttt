...
#                "down": ROOT.TFile.Open(os.path.join(hist_dir, "tttt_jesTotalDown.root"))},
		...
		{"name" : "TTISR", "color" : ROOT.kMagenta,  "type":"theoretical",
				"up" :  ROOT.TFile.Open(os.path.join(hist_dir, "tttt_TTISRUp.root")),
				"down": ROOT.TFile.Open(os.path.join(hist_dir, "tttt_TTISRDown.root"))},
		{"name" : "TTFSR", "color" : ROOT.kMagenta+2,  "type":"theoretical",
				"up" :  ROOT.TFile.Open(os.path.join(hist_dir, "tttt_TTFSRUp.root")),
				"down": ROOT.TFile.Open(os.path.join(hist_dir, "tttt_TTFSRDown.root"))}, 
		{"name" : "DYISR", "color" : ROOT.kMagenta,  "type":"theoretical",
				"up" :  ROOT.TFile.Open(os.path.join(hist_dir, "tttt_DYISRUp.root")),
				"down": ROOT.TFile.Open(os.path.join(hist_dir, "tttt_DYISRDown.root"))},
		{"name" : "DYFSR", "color" : ROOT.kMagenta+2,  "type":"theoretical",
...
mc = [ 
#       {"name": "TTLep_bb", "legendText" : "t#bar{t}b#bar{b}"},
...
#	    plotter.addPDF(sample["name"] , pdf["name"] , pdfHist)
    ...
    for log in [False, True]:
        ...
        plotter.drawNuisances(plot_directory_, log, comparisonPlots = True , texX = plot["name"] )
