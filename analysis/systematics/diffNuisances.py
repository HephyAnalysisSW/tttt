...
parser.add_option("--selection", action="store", default='combined')
parser.add_option('--plot_directory', action='store', default='4t-limits-v2')
...
show =25
n = len(pulls)/show + 1
#plot_directory_ = os.path.join(plot_directory, 'test/pull-plots2/')
plot_directory_ = os.path.join(plot_directory, 'analysisPlots', options.plot_directory , 'RunII', options.selection)
...
if options.plotfile:
  for page in range(n):
    i#canv = ROOT.TCanvas(args.output, args.output)
    n_params = len(pulls[show * page:show * (page + 1)])
    pdata = pulls[show * page:show * (page + 1)]
    print '>> Doing page %i, have %i parameters out of %i' % (page, n_params,len(pulls))

    import ROOT
    fout = ROOT.TFile(options.plotfile,"RECREATE")
    ROOT.gROOT.SetStyle("Plain")
    ROOT.gStyle.SetOptFit(1)
    histogram = ROOT.TH1F("pulls", "Pulls", 60, -3, 10)
    for pull in pulls:
        histogram.Fill(pull)
    ...
    #histogram.GetXaxis().SetRangeUser(show * page,show * (page + 1))
    histogram.Draw("pe")
    ...
    hist_prefit.GetXaxis().SetRangeUser(show * page,show * (page + 1)) 
    hist_prefit.Draw("E2")
    hist_prefit.Draw("histsame")
    gr_fit_b.GetXaxis().SetRangeUser(show * page,show * (page + 1))
    gr_fit_s.GetXaxis().SetRangeUser(show * page,show * (page + 1))
    ROOT.gStyle.SetOptStat(0)
    ...
    if not options.skipFitB:leg.AddEntry(gr_fit_b,"SM-only fit","EPL")
    if not options.skipFitS:leg.AddEntry(gr_fit_s,"SM + EFT fit"   ,"EPL")
    leg.Draw()
    ...
    hist_fit_e_b.GetXaxis().SetRangeUser(show * page,show * (page + 1))
    hist_fit_e_s.GetXaxis().SetRangeUser(show * page,show * (page + 1))
    ...
    leg_rat.AddEntry(hist_fit_e_b,"SM-only fit","F")
    leg_rat.AddEntry(hist_fit_e_s,"SM + EFT fit"   ,"F")
    leg_rat.Draw()
    ...
    specialName = "."
    if options.regex != ".*": specialName = "_"+options.regex+"."
    canvas.Print(plot_directory_ +'/pulls'+specialName+'pdf')
    canvas.Print(plot_directory_ +'/pulls'+specialName+'png')
    canvas_nuis.Print(plot_directory_ +'/nuisances%i'%page+specialName+'pdf')
    canvas_nuis.Print(plot_directory_ +'/nuisances%i'%page+specialName+'png')
    canvas_pferrs.Print(plot_directory_ +'/post_fit_err%i'%page+specialName+'pdf')
    canvas_pferrs.Print(plot_directory_ +'/post_fit_err%i'%page+specialName+'png')
